...
if __name__ == "__main__":
    ...
    with open(Path(res_file_dir, res_file_name), 'w+') as f:
        ...
        for data_set in [train_data_handler, dev_data_handler, test_data_handler]:
            ...
            for value, words in histogram.items():
                if len(words) < min_count:
                    ...
                    logging.info('Ignoring labels: {}'.format(values_to_ignore))
        ...
        # here we can get an error because the label to idx id formed only when it is not present so when dev_data_loader is creating
        # the data set it creates its own filtered data but the label_to_idx is the old one so all the key may not be present for the labels 
